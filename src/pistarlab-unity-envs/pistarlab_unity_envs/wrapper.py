...
def get_action_space(action_spec):
    """
    TODO: Consider adding flattener from https://github.com/Unity-Technologies/ml-agents/blob/main/gym-unity/gym_unity/envs/__init__.py
    """
    action_space = None
    if action_spec.is_discrete():
        ...
        raise UnityGymException(
            "This wrapper does not provide explicit support for both discrete "
            "and continuous actions."
        )
    return action_space


def get_spaces(behavior_specs):
    obs_spaces = {}
    action_spaces = {}

    for name, bspec in behavior_specs.items():
        space_list = []
        space = None
        # Observation Spaces
        for spec in bspec.observation_specs:
            space_list.append(spaces.Box(
                0, 1, dtype=np.float32, shape=spec.shape))
        if len(space_list) > 1:
            space = spaces.Tuple(space_list)
        else:
            space = space_list[0]
        obs_spaces[name] = space

        action_spaces[name] = get_action_space(bspec.action_spec)
    return obs_spaces, action_spaces


class Unity3DEnv(MultiAgentEnv):
    """
    Original Source: https://github.com/ray-project/ray/blob/master/rllib/examples/unity3d_env_local.py
    Modified to generate metadata at start time
    ** For adding new envrionments see: https://github.com/Unity-Technologies/ml-agents/blob/main/docs/Learning-Environment-Executable.md
    ...
    def __init__(self,
        ...
        exec_name = "UnityEnvironment.exe"
        if platform == "win":
            exec_name = "UnityEnvironment.exe"
        elif platform == "linux":
            exec_name = "unity_env_linux.x86_64"
            
        file_name = pkg_resources.resource_filename(__name__,os.path.join("envs",f"{game_name}_{platform}",exec_name))
        logging.info(f"Game File Name: {file_name}")
        super().__init__()
        ...
        self.max_num_players = len(self.possible_players)

        self.last_obs = {}

        self.observation_spaces, self.action_spaces = get_spaces(
            self.unity_env.behavior_specs)
    ...
    def render(self, player_id=None, mode="rgb_array"):
        if player_id is None:
            return None
        obs_space = self.observation_spaces[player_id]
        if obs_space.shape == 3:
            obs = self.last_obs.get(player_id)
